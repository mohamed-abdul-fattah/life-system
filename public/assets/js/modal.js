...
let overlay = document.querySelector('.modall__overlay');
    ...
    // FORWARD ANIMATION 
    let modalActive = async function(){
        let first = await overlayAnimation(1,'block',3,'normal');
        let second = await modalTvEffectAnimation('normal','block',.6);
        let third = await modalTvEffectContentAnimation('flex',.7);
            setTimeout(() => {
               overlay.addEventListener("click", ()=>{ modalReverse() }) 
            }, 500);
    }
    ...
    // REVERSE ANIMATION
    let modalReverse =  async function(){
        // CARD CONTENT 
